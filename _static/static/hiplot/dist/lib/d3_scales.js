...
export function d3_scale_percentile(values) {
    ...
    function invert(y) {
        ...
        var lower = Math.floor(y);
        var upper = Math.ceil(y);
        if (lower == upper) {
            return values[lower];
        }
        var a = y - lower;
...
function wrap_scale(scale_orig, domain_to_scale, scale_to_domain) {
    var scale = function (x) {
        return scale_orig(domain_to_scale(x));
    };
    function domain(new_domain) {
        if (new_domain === undefined) {
            var scale_domain = scale_orig.domain();
    ...
    ;
    function range() {
        var r = scale_orig.range.apply(scale_orig, arguments);
    ...
    scale.ticks.apply = function (_, tickArguments_) {
        var args = [tickArguments_[0]];
        var ta = scale_orig.ticks.apply(scale_orig, args);
...
export function d3_scale_timestamp() {
    ...
    function timestamp_to_jsdate(timestamp) {
        if (timestamp instanceof Date) {
            return timestamp;
        }
        return new Date(timestamp * 1000);
    }
    function jsdate_to_timestamp(date) {
        return date.getTime() / 1000;
    }
    var ts = wrap_scale(d3.scaleTime(), timestamp_to_jsdate, jsdate_to_timestamp);
    return ts;
}
